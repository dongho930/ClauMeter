...
- 게이지 위 **기준선** - 남은 시간 대비 지금쯤 적절한 사용률을 표시하고, 넘어섰으면 초과분(%p)을 함께 알려줍니다
- 게이지마다 **상태 표시** - 5시간과 주간의 안전/주의/위험을 각각 옆에 표시하고, 먼저 막는 쪽을 밝게 강조
- 두 한도 **환산** - "남은 주간 여유 = 5시간 구간 몇 번분", "이 구간을 다 쓰면 이번 주가 몇 %가 되는지"
- 과거 패턴을 학습해 **"이 속도면 ○시간 ○분 뒤 한도 도달"** 과 줄여야 할 속도를 알려주는 AI 조언
