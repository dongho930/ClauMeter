...
const ROWS = [
  ...
  ['notificationsToggleLabel', '사용량 알림 (50%, 75%, 90%)', 'Usage notifications (50%, 75%, 90%)', 'Notificaciones de uso (50%, 75%, 90%)', "Notifications d'utilisation (50 %, 75 %, 90 %)", 'Nutzungsbenachrichtigungen (50 %, 75 %, 90 %)', 'Notificações de uso (50%, 75%, 90%)', '使用量通知 (50%, 75%, 90%)', '使用量通知 (50%、75%、90%)', 'Уведомления об использовании (50%, 75%, 90%)', 'Notifiche di utilizzo (50%, 75%, 90%)', 'Gebruiksmeldingen (50%, 75%, 90%)', 'Powiadomienia o wykorzystaniu (50%, 75%, 90%)'],
  ['notifyFiveHourBody', '5시간 한도 사용률이 {pct}%에 도달했습니다', '5-hour limit usage reached {pct}%', 'El uso del límite de 5 horas alcanzó el {pct}%', "L'utilisation du plafond de 5 heures a atteint {pct} %", 'Die Nutzung des 5-Stunden-Limits hat {pct}% erreicht', 'O uso do limite de 5 horas atingiu {pct}%', '5時間制限の使用率が{pct}%に達しました', '5小时限额使用率已达到{pct}%', 'Использование 5-часового лимита достигло {pct}%', "L'utilizzo del limite di 5 ore ha raggiunto il {pct}%", 'Gebruik van de 5-uurslimiet heeft {pct}% bereikt', 'Wykorzystanie limitu 5-godzinnego osiągnęło {pct}%'],
  ['notifyWeeklyBody', '주간 한도 사용률이 {pct}%에 도달했습니다', 'Weekly limit usage reached {pct}%', 'El uso del límite semanal alcanzó el {pct}%', "L'utilisation du plafond hebdomadaire a atteint {pct} %", 'Die Nutzung des Wochenlimits hat {pct}% erreicht', 'O uso do limite semanal atingiu {pct}%', '週間制限の使用率が{pct}%に達しました', '每周限额使用率已达到{pct}%', 'Использование недельного лимита достигло {pct}%', "L'utilizzo del limite settimanale ha raggiunto il {pct}%", 'Gebruik van de wekelijkse limiet heeft {pct}% bereikt', 'Wykorzystanie limitu tygodniowego osiągnęło {pct}%'],
  ['notifyStatusLineHookSet', 'Claude Code statusLine 훅을 자동으로 등록했습니다', 'Automatically registered the Claude Code statusLine hook', 'Se registró automáticamente el hook statusLine de Claude Code', 'Le hook statusLine de Claude Code a été enregistré automatiquement', 'Der Claude-Code-statusLine-Hook wurde automatisch registriert', 'O hook statusLine do Claude Code foi registrado automaticamente', 'Claude CodeのstatusLineフックを自動的に登録しました', '已自动注册 Claude Code 的 statusLine 钩子', 'Хук statusLine Claude Code зарегистрирован автоматически', "L'hook statusLine di Claude Code è stato registrato automaticamente", 'De Claude Code statusLine-hook is automatisch geregistreerd', 'Automatycznie zarejestrowano hak statusLine Claude Code'],

  // macOS 전용: 다운로드 폴더/DMG에서 바로 실행했을 때 응용 프로그램 폴더로 옮기라고 권하는 대화상자
  ['moveToApplicationsMessage', '클로미터를 응용 프로그램 폴더로 옮길까요?', 'Move ClauMeter to the Applications folder?', '¿Mover ClauMeter a la carpeta Aplicaciones?', 'Déplacer ClauMeter dans le dossier Applications ?', 'ClauMeter in den Programme-Ordner verschieben?', 'Mover o ClauMeter para a pasta Aplicativos?', 'ClauMeterをアプリケーションフォルダに移動しますか?', '要将 ClauMeter 移到“应用程序”文件夹吗?', 'Переместить ClauMeter в папку «Программы»?', 'Spostare ClauMeter nella cartella Applicazioni?', "ClauMeter naar de map Programma's verplaatsen?", 'Przenieść ClauMeter do folderu Aplikacje?'],
  ['moveToApplicationsDetail', '다운로드 폴더나 디스크 이미지(DMG)에서 바로 실행하면 Claude Code와 연동(statusLine 훅)할 수 없습니다. 옮기고 나면 자동으로 다시 실행됩니다.', "ClauMeter can't connect to Claude Code (statusLine hook) while running from Downloads or the disk image (DMG). It will relaunch automatically after moving.", 'Mientras se ejecute desde Descargas o la imagen de disco (DMG), ClauMeter no puede conectarse a Claude Code (hook statusLine). Se volverá a abrir automáticamente después de moverlo.', "Tant qu'il est lancé depuis Téléchargements ou l'image disque (DMG), ClauMeter ne peut pas se connecter à Claude Code (hook statusLine). Il se relancera automatiquement après le déplacement.", 'Solange ClauMeter aus dem Download-Ordner oder dem Disk-Image (DMG) läuft, kann es sich nicht mit Claude Code verbinden (statusLine-Hook). Nach dem Verschieben startet es automatisch neu.', 'Enquanto for executado a partir de Downloads ou da imagem de disco (DMG), o ClauMeter não consegue se conectar ao Claude Code (hook statusLine). Ele será reaberto automaticamente após a mudança.', 'ダウンロードフォルダやディスクイメージ(DMG)から直接実行している間は、Claude Code と連携(statusLine フック)できません。移動後に自動で再起動します。', '从“下载”文件夹或磁盘映像(DMG)直接运行时，无法与 Claude Code 连接(statusLine 钩子)。移动后将自动重新启动。', 'Пока ClauMeter запущен из «Загрузок» или образа диска (DMG), он не может подключиться к Claude Code (хук statusLine). После перемещения он перезапустится автоматически.', "Finché viene eseguito da Download o dall'immagine disco (DMG), ClauMeter non può collegarsi a Claude Code (hook statusLine). Si riavvierà automaticamente dopo lo spostamento.", 'Zolang ClauMeter vanuit Downloads of de schijfkopie (DMG) draait, kan het geen verbinding maken met Claude Code (statusLine-hook). Na het verplaatsen start het automatisch opnieuw.', 'Dopóki ClauMeter działa z folderu Pobrane lub obrazu dysku (DMG), nie może połączyć się z Claude Code (hak statusLine). Po przeniesieniu uruchomi się ponownie automatycznie.'],
  ['moveToApplicationsButton', '응용 프로그램으로 이동', 'Move to Applications', 'Mover a Aplicaciones', 'Déplacer vers Applications', 'In Programme verschieben', 'Mover para Aplicativos', 'アプリケーションに移動', '移到“应用程序”', 'Переместить в «Программы»', 'Sposta in Applicazioni', "Verplaats naar Programma's", 'Przenieś do Aplikacji'],
  ['moveToApplicationsLater', '나중에', 'Not now', 'Ahora no', 'Plus tard', 'Später', 'Agora não', '後で', '以后再说', 'Не сейчас', 'Non ora', 'Niet nu', 'Nie teraz'],
];
